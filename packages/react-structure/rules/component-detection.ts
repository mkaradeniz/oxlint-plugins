...
export const isPascalCase = ({ name }: { name: string | null }) => {
  return name !== null && /^[A-Z]/.test(name) && /[a-z]/.test(name);
};

export const isFunctionLikeExpression = (
  expression: ESTree.Expression | null,
): expression is ESTree.ArrowFunctionExpression | ESTree.Function => {
